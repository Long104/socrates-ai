...
import { openai, MODEL, provider } from "@/lib/openai";
import { SYNTHESIZE_SYSTEM_PROMPT } from "@/lib/prompts";
import type { SynthesizeRequest, SynthesizeResponse } from "@/lib/types";
...
export async function POST(request: Request) {
  try {
    ...
    if (!body.rootBelief || !Array.isArray(body.resolvedAssumptions) || body.resolvedAssumptions.length < 2) {
    ...
    const resolvedList = body.resolvedAssumptions
      .map((a) => `- Fact: ${a.fact} | Original leap: ${a.originalLeap} | User's resolved insight: ${a.resolvedText}`)
      .join("\n");

    const systemPrompt = SYNTHESIZE_SYSTEM_PROMPT
      .replace("{rootBelief}", body.rootBelief)
      .replace("{resolvedList}", resolvedList);
